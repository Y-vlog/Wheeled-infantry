...
{
	hipnuc_raw->len = U2(hipnuc_raw->buf + 2);
...
void HiPNUC_IDLE_Handle(UART_HandleTypeDef* huart, DMA_HandleTypeDef* hdmarx,hipnuc_raw_t *hipnuc_raw)
{
	...
	if((__HAL_UART_GET_FLAG(huart,UART_FLAG_IDLE)!= RESET))//判断串口dma接收�?否�?�于空闲状�?
	{
		__HAL_UART_CLEAR_IDLEFLAG(huart);//清除DMA空闲�?�?标志�?
		
		HAL_UART_DMAStop(huart); //停�??串口dma接收
		
		Data_lave = __HAL_DMA_GET_COUNTER(hdmarx); //获取接收长度
		Data_exist = HIPNUC_MAX_RAW_SIZE - Data_lave; //计算剩余长度

		if(Data_exist >= 0 )  //判断�?否接收到正确长度数据
		{
			/*接收成功后，处理相应接收数据*/
			HiPNUC_Process(hipnuc_raw);
		}
		HAL_UART_Receive_DMA(huart, hipnuc_raw->buf, HIPNUC_MAX_RAW_SIZE);//重新�?动dma接收	
	}
}
